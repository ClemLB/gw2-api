...
@SpringBootTest
class AccountNodesTest {

	@Autowired
	private AccountNodesService service;
	...
	void test3() {
		AtomicReference<List<String>> nodes = new AtomicReference<>();
		assertDoesNotThrow(() -> nodes.set(service.getWithAuthentification(fullApiKey)));
		assertNotNull(nodes.get(), "Service should return informations on nodes");
		assertFalse(nodes.get().isEmpty(), "Service should return informations on nodes");
	}
}
